...
import { BaseService, DialogService } from '@rosoftlab/core';
import { Observable } from 'rxjs';
import { BaseModelFormly } from '../formly/base-model-formly';
import { RslIonicModuleModule } from './rsl-ionic-module.module';
@Component({
  standalone: true,
  selector: 'app-generic-ionic-crud',
...
export class GenericIonicCrudComponent<T extends BaseModelFormly, U extends BaseService<T>> implements OnInit {
  ...
  constructor(
    ...
    this.modelService = injector.get<U>(<any>SERVICE_TOKEN);
    //  =injector.get<U>(U)
    // super(fb, router, route, this.service, dialogService, translate, location);
  ...
  generateForm(model?: T) {
    ...
    const x = this.model.getFormlyFields()
    this.fields = x;
  }
