...
export class GenericIonicCrudComponent<T extends BaseModelFormly, U extends BaseService<T>> implements OnInit {
  ...
  generateForm(model?: T) {
    ...
    this.fields = this.model.getFormlyFields()
  }
          ...
          this.modelService.save(this.baseForm, this.modelId, this.model).subscribe(
            (newModel: T) => {
              ...
              this.afterSave(newModel).subscribe((val: T) => {
                from(this.dialogService.showSaveMessage('Your changes were saved successfully.')).subscribe(d => {
                  fg.markAsPristine();
                });
              });
            },
  ...
  canDeactivate(): Observable<boolean> | boolean {
    ...
    return from(this.dialogService.confirm('Discard changes ?', null, 'Discard'));
  }
