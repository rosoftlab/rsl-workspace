...
export class FileService {
  ...
  constructor(private http: HttpClient, @Inject(SOCKET_URL) socketUrl: string) {
  ...
  uploadFile(file: File | Blob, filename?: string, fileId?: string): Observable<UploadProgress> {
    ...
    return this.http.request<{ file_id: string }>(req).pipe(
      // only let through progress events or the final response
      filter(
        (evt: HttpEvent<{ file_id: string }>): evt is HttpUploadProgressEvent | HttpResponse<{ file_id: string }> =>
          evt.type === HttpEventType.UploadProgress || evt instanceof HttpResponse
      ),
      map((evt) => {
        if (evt.type === HttpEventType.UploadProgress) {
          // live progress
  ...
  downloadFile(fileId: string): Observable<DownloadProgress> {
    ...
    return this.http.request<Blob>(req).pipe(
      ...
      filter(
        (evt: HttpEvent<Blob>): evt is HttpDownloadProgressEvent | HttpResponse<Blob> =>
          evt.type === HttpEventType.DownloadProgress || evt instanceof HttpResponse
      ),
      map((evt) => {
